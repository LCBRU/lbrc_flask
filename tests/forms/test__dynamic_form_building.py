...
def test__add_field__correct_field_type_added(client, faker, field_type_name):
    field = faker.get_test_field(field_type=FieldType._get_field_type(field_type_name))
...
def test__add_field__allowed_file_extensions_not_set(client, faker):
    ...
    assert all(False for v in field_actual.validators if isinstance(v, FileAllowed))
